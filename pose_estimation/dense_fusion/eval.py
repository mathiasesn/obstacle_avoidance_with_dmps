...
def main(opt):
    ...
    estimator.cuda()
    refiner = PoseRefineNet(num_points=num_objects, num_obj=num_objects)
    refiner.cuda()
    ...
    for i, data in enumerate(testdataloader, 0):
        ...
        pred_c = pred_c.view(bs, num_objects)
        ...
        for ite in range(0, iteration):
            ...
            my_mat = quaternion_from_matrix(my_r)
            R = Variable(torch.from_numpy(my_mat[:3, :3].astype(np.float32))).cuda().view(1,3,3)
            my_mat[0:3, 3] = my_t

            new_points = torch.bmm((points - T), T).contiguous()
    ...
    for i in range(num_objects):
        print(f'Object {obj_list[i]} success rate: {float(success_count[i]/num_count[i])}')
        fw.write(f'Object {obj_list[i]} success rate: {float(success_count[i]/num_count[i])}\n')
