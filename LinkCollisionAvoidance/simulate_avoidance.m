...
for i = 1:size(demo,1)-1
    dxe = (demo(i+1, 1:6) - demo(i, 1:6)) ./ dt;
    %dxe = [dxe 0 0 0]';
    ...
    trajpos = getTransform(robot,config,'panda_link7');
    traj = [traj trajpos(1:3,4)];
...
hold on
plot((1:4223)*dt,demo(:,1), 'LineWidth', 1.5)
hold off
grid on
...
hold on
plot((1:4223)*dt,demo(:,2), 'LineWidth', 1.5)
hold off
grid on
xlabel('Time [s]')
ylabel('Y [m]')
...
hold on
plot((1:4223)*dt,demo(:,3), 'LineWidth', 1.5)
hold off
grid on
xlabel('Time [s]')
ylabel('Z [m]')
